...
/**
 * Retrieves a list of physical devices on the system
 * @param {Function} callback
 */
BlockDevice.getDeviceList = function( callback ) {
  
  if( os.platform() !== 'win32' )
    return callback( new Error( 'Only implemented on Win32' ) )
  ...
  child_process.exec(
    ...
    function( error, stdout, stderr ) {
      error == null ?
        void callback( error, parseOutput( stdout ) ) :
        void callback( error )
    }
  )
